...
<svelte:head><title>RutaEscolar | Gestión de transporte</title><meta name="description" content="Gestión de transporte escolar" /></svelte:head>
...
{:else}
  <div class="shell">
    <aside class="sidebar"><div class="brand"><span class="brand-mark">R</span><div><strong>RutaEscolar</strong><small>{session.role}</small></div></div><nav aria-label="Navegación principal"><button class:active={active === 'inicio'} on:click={() => { active = 'inicio'; showForm = false; }}><span>▫</span> Inicio</button><p>{isAdmin ? 'ADMINISTRACIÓN' : 'MI TRANSPORTE'}</p>{#each allowedResources as key}<button class:active={active === key} on:click={() => select(key)}><span>{resources[key].icon}</span> {resources[key].label}</button>{/each}</nav><div class="sidebar-footer"><span class="avatar">{session.name.slice(0, 2).toUpperCase()}</span><div><strong>{session.name}</strong><small>{session.role}</small></div><button class="primary" on:click={logout}>Cerrar sesión</button></div></aside>
    <main><header><div><p class="eyebrow">{session.role.toUpperCase()}</p><h1>{active === 'inicio' ? `Hola, ${session.name}` : config.label}</h1></div><div class="header-actions"><span class="date">{new Intl.DateTimeFormat('es-CO', { dateStyle: 'full' }).format(new Date())}</span><button class="sign-out" on:click={logout}>Cerrar sesión</button></div></header>
      {#if active === 'inicio'}
